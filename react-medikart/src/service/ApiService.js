...
export default class ApiService {
  static BASE_URL = "https://medikart-backend-im3c.onrender.com";
  ...
  static async updateProduct(formData) {
    // Extract the productId from the formData
    const productId = formData.get("productId");

    // Make sure productId exists before making the request
    if (!productId) {
  ...
  static async getAllProductsByCategoryId(categoryId) {
    try {
      ...
      console.log("API Response from getAllProductsByCategoryId:", response);
      return response.data;
    } catch (error) {
      console.error("Error in getAllProductsByCategoryId:", error);
      throw error;
    }
  }
  ...
  static async createOrder(body) {
    ...
    // Check if orderId exists in the response and return it
  ...
  static async verifyPayment(data) {
    return await axios.post("/api/payment/verify-payment", data, {
      headers: {
        "Content-Type": "application/json",
      },
    });
